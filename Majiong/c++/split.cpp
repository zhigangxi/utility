...
bool split::get_hu_info(char* cards)
{
	char gui_num = 0;

	char eye_tbl[CardTypeNum];
	char eye_num = 0;
	char empty = -1;
...
int split::GetHuPaiNeedCardNum(char * cards)
{
	char eye_tbl[CardTypeNum];
	char eye_num = 0;
	for (int i = 0; i < CardTypeNum; i++) {
	...
	for (int i = 0; i < eye_num; i++) {
		...
		{
			cards[eye] = 0;
			needCardNum = ForeachNeedCardNum(cards,eye/9) + 1;
		}
		else
		{
			cards[eye] -= 2;
			needCardNum = ForeachNeedCardNum(cards, eye / 9);
		}
		cards[eye] = n;
		if (minNeed > needCardNum)
			minNeed = needCardNum;
	}
	return minNeed;
}

int split::ForeachNeedCardNum(char *cards, int eye_colorr)
{
	int needCard = 0;
	for (int i = 0; i < 3; i++) {
		needCard += NormalNeedCardNum(cards, i * 9, i * 9 + 8);
	}

	needCard += ZiNeedCardNum(cards);
	return needCard;
}

int split::NormalNeedCardNum(char* cards, int from, int to)
{
	int n = 0;
	...
	int needCard = 0;
	return Split(n,needCard);
}

int split::ZiNeedCardNum(char* cards)
{
	int need_gui = 0;
	for (int i = 27; i < CardTypeNum; i++) {
		int c = cards[i];
		if (c == 0) continue;
		if (c == 1 || c == 4) {
			need_gui = need_gui + 2;
...
int split::Split(int n, int &needCard)
{
	int c = 0;
	while (true) {
		if (n == 0) return 0;
		...
		if (c == 1 || c == 4) {
			return One(n, needCard);
		}
		else if (c == 2) {
			return Two(n, needCard);
		}
	}
	return 0;
}

int split::One(int n, int &needCard)
{
	int c1 = n % 10;
	int c2 = (n % 100) / 10;
	...
	if (n == 0) return needCard;

	return Split(n, needCard);
}

int split::Two(int n, int &needCard)
{
	int c1 = n % 10;
	int c2 = (n % 100) / 10;
	int c3 = (n % 1000) / 100;
