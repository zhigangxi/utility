...
func (server *Server) processMsg() {
	...
	server.RegisterProcFun(uint16(gamepb.GamePbType_EnterGameReq), server.onEnterGame)
	server.RegisterProcFun(uint16(gamepb.GamePbType_CreateUserReq), server.onCreateUser)
	server.RegisterProcFun(uint16(gamepb.GamePbType_PackageReq), server.onPackage)
	server.RegisterProcFun(uint16(gamepb.GamePbType_EquipmentReq), server.onEquipment)
	server.RegisterProcFun(uint16(gamepb.GamePbType_AdminCmdReq), server.onAdminCmd)

	server.RegisterProcFun(uint16(gamepb.GamePbType_PlayerMove), server.onPlayerMove)
	server.RegisterProcFun(uint16(gamepb.GamePbType_EnterScene), server.onEnterScene)
	go func() {
		for {
			server.despatch()
		}
...
func pbToByte(pb proto.Message) []byte {
	...
	name = name[packLen:]
	var cmd = gamepb.GamePbType_TypeEnum_value[name]
