...
func (server *Server) processMsg() {
	...
	server.RegisterProcFun(uint16(loginpb.PbType_LoginReq), server.onLogin)
	server.RegisterProcFun(uint16(loginpb.PbType_RegisterReq), server.onRegister)
	server.RegisterProcFun(uint16(loginpb.PbType_ServerListReq), server.onServerList)
...
func (server *Server) onLogin(sess *network.Session, msg []byte) {
	...
	sendMsg.Err = loginpb.ErrorCode_LoginError
	if server.db.Get(nil, &user) {
		if user.Password == pbMsg.Password {
			sendMsg.Err = loginpb.ErrorCode_NoError
...
func (server *Server) onRegister(sess *network.Session, msg []byte) {
	...
	sendMsg.Err = loginpb.ErrorCode_UserNameExisted
